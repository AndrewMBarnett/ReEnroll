...
function propertyList() {
    ...
    if [[ -f "${reEnrollConfigFile}" ]]; then
        infoOut "Specified ${reEnrollConfigFile} path exists"
    else [[ ! -f "${reEnrollConfigFile}" ]]
    ...
    if [[ ! -f "$reEnrollConfigFile" ]]; then
        ...
        defaults write "$reEnrollConfigFile" ReEnrollVersion "$scriptVersion"
        defaults write "$reEnrollConfigFile" ReEnrollLastRun -date "$timestamp"
        defaults write "$reEnrollConfigFile" DeviceEnrolledStatus "Not Enrolled"
        defaults write "$reEnrollConfigFile" ReEnrollNotificationStatus "No Notification"
        defaults write "$reEnrollConfigFile" ReEnrollMethod "None"
        defaults write "$reEnrollConfigFile" ComputerSiteID "None"
        defaults write "$reEnrollConfigFile" ComputerSite "-1"
    else
...
# Run initial functions and checks
propertyList
rootCheck
dockCheck
...
function enrollDeviceReceipt() {
    ...
    defaults write "$reEnrollConfigFile" DeviceEnrolledStatus "Enrolled Device"
...
function jmfrdeploy() {
    ...
    defaults write "$reEnrollConfigFile" ComputerSiteID "$computerSiteID"
    infoOut "Computer Site Name: $computerSite"
    infoOut "Adding computer site to ReEnroll Config File"
    defaults write "$reEnrollConfigFile" ComputerSite "$computerSite"
    ...
    defaults write "$reEnrollConfigFile" ReEnrollMethod "Redeploy Jamf Framework" 
...
function inventoryError() {
    ...
    defaults write "$reEnrollConfigFile" ReEnrollNotificationStatus "ReEnroll with notification"
    defaults write "$reEnrollConfigFile" ReEnrollMethod "Renewing Enrollment"
        ...
        if [[ "${osMajorVersion}" -ge 12 ]] && [[ -e "/Library/Application Support/Dialog/Dialog.app" ]]; then
            ...
            case ${returncode} in
                ...
                0)  ## Process exit code 0 scenario here
                    ...
                    defaults write "$reEnrollConfigFile" ReEnrollRenewProfilesDialog "Update Now"
                ...
                3)  ## Process exit code 2 scenario here
                    ...
                    defaults write "$reEnrollConfigFile" ReEnrollRenewProfilesDialog "Not Now"
                ...
                4)  ## Process exit code 4 scenario here
                    ...
                    defaults write "$reEnrollConfigFile" ReEnrollRenewProfilesDialog "Allowed timer to expire"
                ...
                20) ## Process exit code 20 scenario here
                    ...
                    defaults write "$reEnrollConfigFile" ReEnrollRenewProfilesDialog "Do Not Disturb enabled"
                    updateDialog "listitem: title: Jamf Update Needed, icon: SF=arrow.clockwise.icloud.fill,weight=bold,"
                    infoOut "Removing enrollment receipt"
                    webhookStatus="ReEnroll with notification"
                ...
                *)  ## Catch all processing
                    ...
                    defaults write "$reEnrollConfigFile" ReEnrollRenewProfilesDialog "Errror: Something else happened"
                    updateDialog "listitem: title: Jamf Update Needed, icon: SF=arrow.clockwise.icloud.fill,weight=bold,"
                    infoOut "Removing enrollment receipt"
                    webhookStatus="ReEnroll with notification"
...
# Determine if we should rotate the password (evaluate NOW, not later)
...
# create jamf-recon.zsh script with the pre-determined command and proper logging
...
function reEnrollInvitation() {
    ...
    if [ "$enrollmentInvitation" = "" ] || [ "$sendEnrollmentInvitation" = "false" ]; then
        ...
        defaults write "$reEnrollConfigFile" ReEnrollMethod "Enrollment Invitation"
...
function updatedComputerInventoryInfo () {
    ...
    function computerSiteUpdate () {
        ...
        # Update Computer Site to original site
        ...
        originalComputerSite=$(defaults read "$reEnrollConfigFile" ComputerSite)
        originalComputerSiteID=$(defaults read "$reEnrollConfigFile" ComputerSiteID)
